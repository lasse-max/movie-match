...
describe("bridge", () => {
  it("prefers a title fitting BOTH anchors over a higher-rated one-lane pick", async () => {
    ...
    const result = await bridge([matrix, equalizer], [], [], [878], [28], false, null);
  ...
  it("falls back when nothing clears the quality floor", async () => {
    const fallback = { id: 99, title: "FB", year: null, posterUrl: null, genreIds: [] };
    const result = await bridge([pm(1, [878, 28], 5.0)], [], [], [878], [28], false, fallback);
